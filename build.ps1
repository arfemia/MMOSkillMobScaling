# Build + install the MMO Mob Scaling plugin jar in one shot.
#
# Self-contained (mirrors the ziggfreed-common build.ps1): gradlew build, pin the
...
Write-Host "`n=== Building MmoMobScaling (gradlew build) ===" -ForegroundColor Cyan
...
# 'MmoMobScaling-*.jar' - that also matches the -sources.jar / -javadoc.jar siblings.
